...
<div class="card-body">
    @section('content')
        <div class="text-right">
            <a href="{{ route('E_BookCreate') }}" class="btn btn-lg btn-primary m-2"><i class="fas fa-plus"></i> Create Post</a>
        </div>
                    ...
                    @if ($books)
                        @foreach ($books as $book)
                            <tr>
                                    ...
                                    <div class="form-group">
                                        <label class="custom-switch mt-2">
                                            <input type="checkbox" name="custom-switch-checkbox"
                                                id="statusCheckbox_{{ $book->id }}" data-item-id="{{ $book->id }}"
                                ...
                                <td>
                                    <a href="
                                    {{ route('E_bookEdit', $book->id) }}
                                    " class="btn btn-primary"><i
                                            class="fas fa-edit"></i> Edit</a>

                                    <button type="button" class="btn btn-danger deleteBtn"
                                    data-id="{{ $book->id }}"
                                        data-toggle="modal" data-target="#E_BookDeleteModel">
    ...
    @endsection
    {{-- -- delete Blog Modal   --}}
    <div class="modal fade" id="E_BookDeleteModel" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <h3 class="mt-4 text-center">Delete Blog</h3>
                <form id="E_BookDelete">
                    @csrf
...
<script>
    $(document).ready(function() {
        $('input[type="checkbox"]').on('change', function() {
            ...
            $.ajax({
                url: 'E_book/' + itemId + '/StatusUpdateE_book',
                type: 'POST',
                data: {
                    status: isActive
                },
...
<script>
    ...
    $('#E_BookDelete').submit(function(e) {
        e.preventDefault();
